...
| 全量端到端（**D-014 复跑：15 趟**，同一份输入） | 对账通过 | ✅ 见 4.2/4.3 |
...
>
> **D-014 复跑（2026-09-24，任务 `d014-full-01`）**：把作业拓扑从 41 趟压到
> **15 趟**（单趟双流 K/Q/D + 每侧一趟评分，见 decisions.md D-014）后，
> 用**同一份输入**重跑，三表哈希与上表**完全相同**
> （`ffe09a5229c1b9d3` / `3a78626f2506d791` / `dcfcfd3212d18ae3`），
> 规则命中数与 fix 计数器同样逐条一致 —— 说明趟数优化**没有改变任何语义**。
> 端到端耗时 **7 分 48 秒**（旧 41 趟约 50 分钟，约 6 倍）。
...
| fix | 本次运行的 4 个计数器因「各趟日志互相覆盖」缺陷丢失；缺陷已修，下次运行完整上报。四个值已在 §3.1 的全量本地黄金测试与样本规模集群逐趟对账中逐条验证。**D-014 复跑（15 趟）已完整上报**：P1_text 48 / R4_ms 13,503 / M2_strip 47 / U3_zip_plus4 73 —— 全部一致 |
